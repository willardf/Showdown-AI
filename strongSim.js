...
function BattlePokemon(set, side) {
	...
	this.baseTemplate = selfB.getTemplate(set.species || set);
	if (typeof set === 'string') {
		set = selfB.randomSet(this.baseTemplate, side.pokemon.length);
	}
...
function Battle(roomid, format) {
	...
	this.makeRequest = function(type, requestDetails) {
		...
		case 'switch':
			...
			if (switchTable.any(true)) {
				...
				p1request = {reqside:'p1', forceSwitch: switchTable, side: selfB.p1.getData(), rqid: selfB.rqid};
				async.nextTick(async.apply(selfB.switchCallback, p1request));
			}
			...
			if (switchTable.any(true)) {
				...
				p2request = {reqside:'p2', forceSwitch: switchTable, side: selfB.p2.getData(), rqid: selfB.rqid};
				async.nextTick(async.apply(selfB.switchCallback, p2request));
			}
			break;
		...
		case 'teampreview':
			...
			p1request = {reqside:'p1', teamPreview: true, side: selfB.p1.getData(), rqid: selfB.rqid};
			async.nextTick(async.apply(selfB.teamprevCallback, p1request));
			
			selfB.p2.currentRequest = 'teampreview';
			p2request = {reqside:'p2', teamPreview: true, side: selfB.p2.getData(), rqid: selfB.rqid};
			async.nextTick(async.apply(selfB.teamprevCallback, p1request));
		...
		default:
			...
			p1request = {reqside:'p1', active: activeData, side: selfB.p1.getData(), rqid: selfB.rqid};
			async.nextTick(async.apply(selfB.moveCallback, p1request));

			selfB.p2.currentRequest = 'move';
			activeData = selfB.p2.active.map(function(pokemon) {
				if (pokemon) return pokemon.getRequestData();
			});
			p2request = {reqside:'p2', active: activeData, side: selfB.p2.getData(), rqid: selfB.rqid};
			async.nextTick(async.apply(selfB.moveCallback, p2request));
			break;
		}
		...
		if (p1request) {
			//selfB.p1.emitUpdate({ side: 'p1', request: p1request });
		} else {
		...
		if (p2request) {
			//selfB.p2.emitUpdate({ side: 'p2', request: p2request });
		} else {
	...
	this.win = function(side) {
		async.nextTick(async.apply(selfB.winCallback, {'reqside':side.id}));
		var winSide = false;
		if (selfB.ended) {
			return false;
		}
	...
	this.go = function() {
		...
		while (selfB.queue.length) {
			...
			if (selfB.currentRequest) return;
